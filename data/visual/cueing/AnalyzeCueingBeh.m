...
for i_sub = 1:n_subs
    ...
    count_rt(i_sub,i_sesh,:) = [length(rt(validity == 1 & rt >= rt_toofast & rt <= rt_tooslow)) length(rt(validity == 0 & rt >= rt_toofast & rt <= rt_tooslow))];
    median_rt(i_sub,i_sesh,:) = [nanmedian(rt(validity == 1 & rt >= rt_toofast & rt <= rt_tooslow)) nanmedian(rt(validity == 0 & rt >= rt_toofast & rt <= rt_tooslow))];
    mean_accuracy(i_sub,i_sesh,:) = [sum(accuracy(validity == 1 & rt >= rt_toofast & rt <= rt_tooslow))/sum(validity == 1 & rt >= rt_toofast & rt <= rt_tooslow) sum(accuracy(validity == 0 & rt >= rt_toofast & rt <= rt_tooslow))/sum(validity == 0 & rt >= rt_toofast & rt <= rt_tooslow)];
